...
public class PdaInboundSortationManagerImpl extends BaseManagerImpl implements PdaInboundSortationManager {
    ...
    public PdaInboundSortationCommand pdaScanNewContainer(PdaInboundSortationCommand pdaInboundSortationCommand) throws Exception {
        ...
        if (container.getLifecycle().equals(ContainerStatus.CONTAINER_LIFECYCLE_OCCUPIED)) {
            if (!container.getStatus().equals(ContainerStatus.CONTAINER_STATUS_CAN_PUTAWAY) && !container.getStatus().equals(ContainerStatus.CONTAINER_STATUS_USABLE)) {
                log.warn("pdaScanNewContainer container status error =" + container.getStatus() + " logid: " + pdaInboundSortationCommand.getLogId());
                throw new BusinessException(ErrorCodes.PDA_INBOUND_SORTATION_STATUS_ERROR, new Object[] {container.getStatus()});
            }
        }
        if (container.getLifecycle().equals(ContainerStatus.CONTAINER_LIFECYCLE_OCCUPIED)) {
        ...
        if (!pdaInboundSortationCommand.getIsSortationContainerAssign()) {
            // 如果不允许用户共享分拣目标容器
            List<WhContainerAssign> cList = whContainerAssignDao.findWhContainerAssignByContainerId(pdaInboundSortationCommand.getOuId(), container.getId());
            for (WhContainerAssign ca : cList) {
                // 验证是否存在当前用户和分拣目标容器表中不相同的数据
                if (!pdaInboundSortationCommand.getUserId().equals(ca.getUserId())) {
                    log.warn("pdaScanNewContainer pdaInboundSortationCommand.getUserId() != WhContainerAssign.getUserId logid: " + pdaInboundSortationCommand.getLogId());
                    throw new BusinessException(ErrorCodes.PDA_INBOUND_CONTAINER_ASSIGN_USER_ERROR);
                }
            }
        }
        // 保留目标容器ID
    ...
    private PdaInboundSortationCommand pdaScanNewContainerInsertSkuInv(PdaInboundSortationCommand pdaInboundSortation) throws Exception {
        ...
        // 获取商品属性UUID 用户保存到t_wh_container_assign
        String uuidCa = SkuInventoryUuid.invUuid(newSkuInv);
        // 内部容器号=目标容器号
        newSkuInv.setInsideContainerId(pdaInboundSortation.getNewContainerId());
        // 查询目标容器是否有外部容器号(托盘信息)
        List<WhSkuInventory> invList = whSkuInventoryDao.findWhSkuInventoryByContainerIdLocationIsNull(pdaInboundSortation.getOuId(), pdaInboundSortation.getNewContainerId(), null);
        if (invList.size() > 0) {
            // 有对应的外部容器号
            // 给移入的库存记录添加外部容器ID
        ...
        // 判断t_wh_container_assign表中是否存在对应数据
        ...
        // 验证容器库存是否分拣完成 修改对应容器库存状态
        updateOutInContainer(pdaInboundSortation, outerContainerId, b);
     ...
     * @throws Exception
     */
    private PdaInboundSortationCommand updateOutInContainer(PdaInboundSortationCommand pdaInboundSortation, Long outerContainerId, boolean b) throws Exception {
        // 如果有外部容器信息
        if (null != outerContainerId) {
            // 根据外部容器号 查找容器库存数据 location is null 查询多条对应的库存数据
            List<WhSkuInventory> outInvList = whSkuInventoryDao.findWhSkuInventoryByContainerIdLocationIsNull(pdaInboundSortation.getOuId(), null, outerContainerId);
            if (outInvList.size() == 0) {
                // 此原始容器号对应外部容器库存已完成全部商品分拣
                ...
                pdaInboundSortation.setContainerId(outerContainerId);
                // 如果原始容器库存包含外部容器 需要把外部容器状态改为可用
                b = updateContainerLifecycleAndStatus(pdaInboundSortation);
                if (!b) {
                    // 修改箱信息容器状态失败
                    throw new BusinessException(ErrorCodes.SYSTEM_ERROR);
