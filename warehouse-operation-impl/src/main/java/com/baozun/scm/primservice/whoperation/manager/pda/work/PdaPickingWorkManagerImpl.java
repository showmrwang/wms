...
public class PdaPickingWorkManagerImpl extends BaseManagerImpl implements PdaPickingWorkManager {
    ...
    public PickingScanResultCommand judgeSkuAttrIdsIsUnique(PickingScanResultCommand command) {
        ...
        String skuBarcode = command.getTipSkuBarCode(); // 商品条码
    ...
    private void statisticOpertaionLineIdQty(Long locationId, Long outerContainerId, Long insideContainerId, String skuAttrId, Double qty, Long operationId, Long ouId, Boolean isShortPikcing) {
        ...
        for (WhOperationLineCommand oLCmd : operLineList) {
            ...
            if (skuAttrId.equals(opLskuAttrId) && locationId.longValue() == oLCmd.getFromLocationId().longValue() && (outerContainerId == null ? true : outerContainerId.equals(oLCmd.getFromOuterContainerId()))
                    && (insideContainerId == null ? true : insideContainerId.equals(oLCmd.getFromInsideContainerId()))) {
                ...
                if (isShortPikcing) { // 短拣
                    if (null != insideContainerId) { // 当前sku是内部容器内的sku
                        ...
                        operLineCacheCmd.setLocPickingOperIds(locShortPikcingOperIds);
                    }
                } else { // 非短拣
                    if (null != insideContainerId) {
                        ...
                        operLineCacheCmd.setInsidePickingOperIds(locPickingOperIds);
                    }
                }
