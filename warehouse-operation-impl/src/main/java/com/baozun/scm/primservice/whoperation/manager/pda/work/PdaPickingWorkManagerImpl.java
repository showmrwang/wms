...
public class PdaPickingWorkManagerImpl extends BaseManagerImpl implements PdaPickingWorkManager {
    ...
    private void reduceQty(Long skuId, Long ouId, String outboundboxCode) {
        ...
        if (null != invList && invList.size() != 0) {
            ...
            Long oldInvId = inv.getId();
            Double oldQty = inv.getOnHandQty();
    ...
    public PickingScanResultCommand tipSku(PickingScanResultCommand command, String operationWay) {
        ...
        // Map<Long, Map<String, Set<Integer>>> insideSkuAttrIdsContainerLattice =
        // operatorLine.getInsideSkuAttrIdsContainerLattice();
        // Map<Long, Map<String, Set<Integer>>> skuAttrIdsContainerLattice =
        // operatorLine.getSkuAttrIdsContainerLattice();
    ...
    // cacheManager.setObject(CacheConstants.CACHE_LATTICE_NO + operationId.toString(), latticeList,
    ...
    public PickingScanResultCommand scanSku(PickingScanResultCommand command, WhSkuCommand skuCmd, Boolean isTabbInvTotal, String operationWay) {
        ...
        String tipTurnoverBoxCode = command.getTipTurnoverBoxCode();
        String tipOutBoundBoxCode = command.getTipOutBounxBoxCode();
        ...
        if (Constants.PICKING_INVENTORY.equals(operationWay)) { // 拣货(库位库存变成容器库存)
            if (pickingWay == Constants.PICKING_WAY_ONE || pickingWay == Constants.PICKING_WAY_TWO) { // 小车+小车出库箱
                ...
                if (null != insideContainerId) {
                    ...
                    // insideSkuAttrIdsLattice =
                    // insideSkuAttrIdsContainerLattice.get(insideContainerId);
                }
        ...
        CheckScanResultCommand cSRCmd =
                pdaPickingWorkCacheManager.pdaPickingyCacheSkuAndCheckContainer(latticeNo, pickingWay, latticeSkuQty, latticeInsideSkuQty, operationWay, ouId, operLocSkuIds, insideSkuAttrIdsSnDefect, skuAttrIdsSnDefect, insideSkuAttrIds,
                        locSkuAttrIdsQty, skuAttrIds, scanPattern, locationIds, locSkuQty, locationId, icSkuIds, outerContainerIds, outerContainerCmd, operationId, insideContainerSkuIds, insideContainerIds, locInsideContainerIds, insideContainerCmd,
                        skuCmd);
        if (cSRCmd.getIsContinueScanSn()) {
            ...
            if (Constants.PICKING_INVENTORY.equals(operationWay)) { // 拣货(库位库存变成容器库存)
                if (Constants.PICKING_WAY_FIVE != pickingWay && Constants.PICKING_WAY_SIX != pickingWay) {
                    ...
                    List<WhOperationExecLine> execLineList =
                            this.addPickingOperationExecLine(isTurnkful,tipTurnoverBoxCode, tipOutBoundBoxCode, containerId, isShortPickingEnd, command.getScanPattern(), pickingWay, operationWay, skuAttrIds, locationId, isShortPikcing, userId, outBoundBoxId,
                                    outBoundBoxCode, turnoverBoxId, outerContainerId, insideContainerId, operationId, ouId, skuCmd.getScanSkuQty(), latticeNo);
                    // 添加容器库存
            ...
            if (Constants.REPLENISHMENT_PICKING_INVENTORY.equals(operationWay)) { // 补货中的拣货(库位库存变成容器库存)
                if (Constants.REPLENISH_WAY_TWO != pickingWay && Constants.REPLENISH_WAY_THREE != pickingWay) {
                    // 添加作业执行明细
                    List<WhOperationExecLine> execLineList =
                            this.addPickingOperationExecLine(isTurnkful,tipTurnoverBoxCode, tipOutBoundBoxCode, containerId, isShortPickingEnd, command.getScanPattern(), pickingWay, operationWay, skuAttrIds, locationId, isShortPikcing, userId, outBoundBoxId,
                                    outBoundBoxCode, turnoverBoxId, outerContainerId, insideContainerId, operationId, ouId, skuCmd.getScanSkuQty(), latticeNo);
            ...
            if (Constants.INVMOVE_PICKING_INVENTORY.equals(operationWay)) { // 库内移动中的拣货(库位库存变成容器库存)
                // 添加作业执行明细
                // List<WhOperationExecLine> execLineList =
                this.addInvMoveOperationExecLine(isShortPickingEnd, command.getScanPattern(), pickingWay, skuAttrIds, locationId, isShortPikcing, userId, outBoundBoxId, outBoundBoxCode, turnoverBoxId, outerContainerId, insideContainerId, operationId,
            ...
            if (Constants.REPLENISHMENT_PICKING_INVENTORY.equals(operationWay)) { // 补货中的拣货(库位库存变成容器库存)
                if (Constants.REPLENISH_WAY_TWO != pickingWay && Constants.REPLENISH_WAY_THREE != pickingWay) {
                    ...
                    whSkuInventoryManager.replenishmentContainerInventory(execLineList, isShortPikcing, snList, skuAttrIds, locationId, operationId, ouId, outerContainerId, insideContainerId, turnoverBoxId, isTabbInvTotal, userId, workCode,
                            skuCmd.getScanSkuQty(), pickingWay);
                } else if (Constants.REPLENISH_WAY_TWO == pickingWay) {
                    this.wholeCaseOperationExecLine(command, outerContainerId, null, isTabbInvTotal);
                }
            }
            if (Constants.INVMOVE_PICKING_INVENTORY.equals(operationWay)) { // 库内移动中的拣货(库位库存变成容器库存)
                // 添加作业执行明细
            ...
            if (Constants.REPLENISHMENT_PICKING_INVENTORY.equals(operationWay)) { // 补货中的拣货(库位库存变成容器库存)
                if (Constants.REPLENISH_WAY_TWO != pickingWay && Constants.REPLENISH_WAY_THREE != pickingWay) {
                    ...
                    whSkuInventoryManager.replenishmentContainerInventory(execLineList, isShortPikcing, snList, skuAttrIds, locationId, operationId, ouId, outerContainerId, insideContainerId, turnoverBoxId, isTabbInvTotal, userId, workCode,
                            skuCmd.getScanSkuQty(), pickingWay);
                } else if (Constants.REPLENISH_WAY_TWO == pickingWay && Constants.REPLENISH_WAY_THREE == pickingWay) {
                    this.wholeCaseOperationExecLine(command, outerContainerId, insideContainerId, isTabbInvTotal);
                }
            }
            if (Constants.INVMOVE_PICKING_INVENTORY.equals(operationWay)) { // 库内移动中的拣货(库位库存变成容器库存)
            ...
            if (Constants.PICKING_INVENTORY.equals(operationWay)) { // 拣货(库位库存变成容器库存)
                ...
                // 更改出库单状态
                this.changeOdoStatus(pickingMode, operationId, ouId, userId);
            }
            // 修改小车状态
            if (Constants.PICKING_INVENTORY.equals(operationWay) && (pickingWay == Constants.PICKING_WAY_ONE || pickingWay == Constants.PICKING_WAY_TWO)) {
    ...
    public String insertIntoCollection(PickingScanResultCommand command, Long ouId, Long userId) {
        ...
        if (Constants.WH_PICKING_MODE.equals(work.getPickingMode())) {
            ...
            // this.pdaConcentrationManager.insertIntoSeedingCollectionNew(command.getBatch(),
            // work.getId(), execLineCommandList, ouId, userId);
    ...
    // // 容器Lifecycle无效
    // log.error("pdaScanContainer container lifecycle error =" + c.getLifecycle() + " logid: " +
    // logId);
    // throw new BusinessException(ErrorCodes.OUT_BOUNDBOX_IS_NOT_NORMAL);
    // }
    // // 验证容器状态是否是待上架
    // if (!(c.getStatus().equals(ContainerStatus.CONTAINER_STATUS_REC_OUTBOUNDBOX) ||
    // c.getStatus().equals(ContainerStatus.CONTAINER_STATUS_PICKING))) {
    // log.error("pdaScanContainer container status error =" + c.getStatus() + " logid: " + logId);
    // throw new BusinessException(ErrorCodes.OUT_BOUNDBOX_IS_NOT_NORMAL, new Object[]
    // {c.getStatus()});
    // }
     ...
     */
    private List<WhOperationExecLine> addPickingOperationExecLine(Boolean isTurnkful,String tipTurnoverBoxCode, String tipOutBoundBoxCode, Long containerId, Boolean isShortPickingEnd, Integer scanPattern, Integer pickingWay, String operationWay, String skuAttrId,
            Long locationId, Boolean isShortPicking, Long userId, Long outBoundBoxId, String outBoundBoxCode, Long turnoverBoxId, Long outerContainerId, Long insideContainerId, Long operationId, Long ouId, Double qty, Integer latticeNo) {
        ...
        if ((Constants.PICKING_INVENTORY.equals(operationWay) && Constants.PICKING_WAY_FIVE == pickingWay)) {
            list = this.palletSaveExecLine(operationId, insideContainerId, ouId, operLineList, userId, locationId, outerContainerId);
        } else if ((Constants.PICKING_INVENTORY.equals(operationWay) && Constants.PICKING_WAY_SIX == pickingWay)) {
            list = this.containerSaveExecLine(operationId, insideContainerId, ouId, operLineList, userId, locationId);
        } else {
            list =
                    this.splitContainerSaveExecLine(isTurnkful,tipTurnoverBoxCode, tipOutBoundBoxCode, containerId, latticeNo, isShortPickingEnd, scanPattern, pickingWay, skuAttrId, locationId, isShortPicking, userId, outBoundBoxId, outBoundBoxCode, turnoverBoxId,
                            outerContainerId, insideContainerId, operationId, ouId, qty, operLineList, operationWay);
        }
    ...
    private List<WhOperationExecLine> splitContainerSaveExecLine(Boolean isTurnkful,String tipTurnoverBoxCode, String tipOutBoundBoxCode, Long containerId, Integer latticeNo, Boolean isShortPickingEnd, Integer scanPattern, Integer pickingWay, String skuAttrId,
            Long locationId, Boolean isShortPicking, Long userId, Long outBoundBoxId, String outBoundBoxCode, Long turnoverBoxId, Long outerContainerId, Long insideContainerId, Long operationId, Long ouId, Double qty,
            List<WhOperationLineCommand> operLineList, String operationWay) {
        List<WhOperationExecLine> list = new ArrayList<WhOperationExecLine>();
        ...
        for (WhOperationLineCommand oLCmd : operLineList) {
            ...
            if (Constants.PICKING_INVENTORY.equals(operationWay) && (pickingWay == Constants.PICKING_WAY_ONE || pickingWay == Constants.PICKING_WAY_TWO)) {// 出库箱流程
            ...
            if (skuAttrId.equals(opLskuAttrId) && locationId.longValue() == oLCmd.getFromLocationId().longValue() && ioIds.equals(lineioIds)) {
                ...
                if (Constants.PICKING_INVENTORY.equals(operationWay) && pickingWay == Constants.PICKING_WAY_THREE) {// 出库箱流程
                    OperatioLineStatisticsCommand operatorLine = cacheManager.getObject(CacheConstants.OPERATIONLINE_STATISTICS + operationId.toString());
                    if (null == operatorLine) {
                        throw new BusinessException(ErrorCodes.COMMON_CACHE_IS_ERROR);
                    }
                ...
                if (Constants.PICKING_INVENTORY.equals(operationWay) && pickingWay == Constants.PICKING_WAY_FOUR) {// 周转箱流程
                    OperatioLineStatisticsCommand operatorLine = cacheManager.getObject(CacheConstants.OPERATIONLINE_STATISTICS + operationId.toString());
                    if (null == operatorLine) {
                        throw new BusinessException(ErrorCodes.COMMON_CACHE_IS_ERROR);
                    }
                ...
                if (Constants.PICKING_INVENTORY.equals(operationWay) && pickingWay == Constants.PICKING_WAY_ONE) {// 小车
                    OperatioLineStatisticsCommand operatorLine = cacheManager.getObject(CacheConstants.OPERATIONLINE_STATISTICS + operationId.toString());
                    if (null == operatorLine) {
                        throw new BusinessException(ErrorCodes.COMMON_CACHE_IS_ERROR);
                    }
                ...
                if (Constants.PICKING_INVENTORY.equals(operationWay) && pickingWay == Constants.PICKING_WAY_TWO) {// 小车加出库箱
                    OperatioLineStatisticsCommand operatorLine = cacheManager.getObject(CacheConstants.OPERATIONLINE_STATISTICS + operationId.toString());
                    if (null == operatorLine) {
                        throw new BusinessException(ErrorCodes.COMMON_CACHE_IS_ERROR);
                    }
                ...
                if (null != operationLineId) {
                    ...
                    Double oLqty = oLCmd.getQty()-oLCmd.getCompleteQty();
                    if (qty.doubleValue() > oLqty.doubleValue()) {// 扫描的数量大于计划量
                        sum += oLqty;
                        Double subtract = qty.doubleValue() - sum;
                        if (subtract.doubleValue() > 0) {
                            // line.setCompleteQty(oLCmd.getQty());
                            whOperationExecLine.setQty(oLqty);
                            ...
                            // 修改作业明细表
                            Double lineQty = oLqty+line.getCompleteQty();
                            line.setCompleteQty(lineQty);
                        ...
                        if (subtract.doubleValue() < 0) {
                            Double lineCompleteQty = sum - qty.doubleValue();
                            line.setCompleteQty(lineCompleteQty);
                            Double execQty = qty.doubleValue() - (sum - oLqty);
                            whOperationExecLine.setQty(execQty); // Bug
                            whOperationExecLineDao.insert(whOperationExecLine);
                            insertGlobalLog(GLOBAL_LOG_INSERT, whOperationExecLine, ouId, userId, null, null);
                            list.add(whOperationExecLine);
                        }

                    } else if (qty.doubleValue() < oLqty.doubleValue()) {
    ...
    private WhOperationExecLine getWhOperationExecLine(Long userId, String outBoundBoxCode, Long turnoverBoxId, Long outBoundBoxId, Long operationId, Long ouId, Long operationLineId, Long outerContainerId, Long insideContainerId) {
        ...
        // if (!StringUtils.isEmpty(outBoundBoxCode) && null != outBoundBoxId) { //
        // 判断当前的出库箱和拣货的出库箱是否一致
    ...
    public PickingScanResultCommand palletPickingOperationExecLine(PickingScanResultCommand command, Boolean isTabbInvTotal) {
        ...
        if (1 == command.getPalletPickingMode() || 2 == command.getPalletPickingMode()) {
            cSRCmd = pdaPickingWorkCacheManager.wholeCaseCacheAndCheck(command.getLocationId(), outerContainerId, null, operationId);
        } else {
            String insideContainerCode = command.getTipInsideContainerCode();
            Long insideContainerId = null;
            ContainerCommand insideContainerCmd = null;
        ...
        if (cSRCmd.getIsNeedTipLoc()) {
            Long locationId = cSRCmd.getTipLocationId();
            Location location = whLocationDao.findByIdExt(locationId, command.getOuId());
            if (null == location) {
                throw new BusinessException(ErrorCodes.PDA_MAN_MADE_PUTAWAY_LOCATION_NULL);
            }
            command.setTipLocationBarCode(location.getBarCode());
     ...
     * @return
     */
    public PickingScanResultCommand wholeCaseOperationExecLine(PickingScanResultCommand command, Long outerContainerId, Long insideContainerId, Boolean isTabbInvTotal) {
        // 根据作业id获取作业明细信息
        ...
        if (null != outerContainerId) {
            // 修改托盘状态
            Container container = new Container();
            ContainerCommand containerCmd = containerDao.findContainerCommandByIdExt(outerContainerId, command.getOuId());
            if (null == containerCmd) {
                throw new BusinessException(ErrorCodes.PDA_INBOUND_SORTATION_CONTAINER_NULL);
            }
            BeanUtils.copyProperties(containerCmd, container);
            ...
            Map<Long, Set<Long>> outerToInside = new HashMap<Long, Set<Long>>();
            outerToInside = operatorLine.getOuterToInside();
            Set<Long> insides = outerToInside.get(outerContainerId);
            for (Long inside : insides) {
                Container insideContainer = new Container();
                ContainerCommand insideContainerCmd = containerDao.findContainerCommandByIdExt(inside, command.getOuId());
                if (null == insideContainerCmd) {
                    throw new BusinessException(ErrorCodes.PDA_INBOUND_SORTATION_CONTAINER_NULL);
                }
                BeanUtils.copyProperties(insideContainerCmd, insideContainer);
                container.setLifecycle(ContainerStatus.CONTAINER_LIFECYCLE_OCCUPIED);
                if (null != command.getPickingWay()) {
                    insideContainer.setStatus(ContainerStatus.CONTAINER_STATUS_PICKING_END);
                } else {
     ...
     * @param workId
     */
    public void removeCache(Long workId, Long ouId, Long operId) {
        // 根据工作Id和ouId获取作业信息
        ...
        if (null != workId) {
            ...
            operationId = WhOperationCommand.getId();
        } else {
            operationId = operId;
        }
    ...
    public PickingScanResultCommand toWholeCase(PickingScanResultCommand command, Boolean isTabbInvTotal, String operationWay) {
        ...
        if (!StringUtils.isEmpty(command.getTipOuterContainerCode())) {
            if (2 == command.getTempReplenishWay() && (1 == command.getPalletPickingMode() || 2 == command.getPalletPickingMode())) {
                command = this.palletPickingOperationExecLine(command, isTabbInvTotal);
            } else {
                // 提示内部容器
                command = this.tipInsideContainer(command);
            }
        }
        if (!StringUtils.isEmpty(command.getTipInsideContainerCode())) {
            if ((2 == command.getTempReplenishWay() && (3 == command.getPalletPickingMode() || 4 == command.getPalletPickingMode()))
                    || (3 == command.getTempReplenishWay() && (3 == command.getContainerPickingMode() || 4 == command.getContainerPickingMode()))) {
                command = this.containerPickingOperationExecLine(command, isTabbInvTotal);
            } else {
                // 提示SKU
                command = this.tipSku(command, Constants.REPLENISHMENT_PICKING_INVENTORY);
            }
        }
        return command;
    }
    ...
    public void changeOdoStatus(String pickingMode, Long operationId, Long ouId, Long userId) {
        ...
        for (WhOperationLineCommand whOperationLineCommand : whOperationLineCommandLst) {
            ...
            if (null != whOdoLine && !OdoLineStatus.CANCEL.equals(whOdoLine.getOdoLineStatus()) && 0 == odoOutBoundBoxByOdoLine.size() && 0 == operationCommandByOdoLine.size()) {
                int odoLineStatus = Integer.parseInt(whOdoLine.getOdoLineStatus());
                if (Constants.WH_PICKING_MODE.equals(pickingMode)) {
                    if (30 > odoLineStatus) {
