package com.baozun.scm.primservice.whoperation.excel.xml;
...
import com.baozun.scm.primservice.whoperation.excel.ResolveFieldValueConverter;
		...
		if (fieldNode != null) {
			for (int i = 0; i < fieldNode.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					String nullErrorCode = fieldEle.getAttribute("nullErrorCode");
					if(StringUtils.isNotBlank(nullErrorCode)){
						fieldValue.setNullErrorCode(nullErrorCode);
					}
					...
					String regexErrCode = fieldEle.getAttribute("regexErrCode");
					if(StringUtils.isNotBlank(regexErrCode)){
						fieldValue.setRegexErrCode(regexErrCode);
					}
					//对齐方式
