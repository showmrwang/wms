...
public class PdaSysSuggestPutwayManagerImpl extends BaseManagerImpl implements PdaSysSuggestPutwayManager{
    ...
    */
    ...
    private ScanResultCommand containerPutWayNoLocation(InventoryStatisticResultCommand  isrCommand,Long ouId,Long userId,ScanResultCommand srCmd,Long funcId,ContainerCommand containerCmd,List<WhSkuInventoryCommand>  invList,Warehouse warehouse,Integer  putawayPatternDetailType) {
        log.info("PdaSysSuggestPutwayManagerImpl updateContainer is start"); 
        Long containerId = containerCmd.getId();  //货箱id
        String outContainerCode = containerCmd.getCode();  //货箱号
    ...
    public ScanResultCommand splitUserSuggestLocation(String outContainerCode,String locationCode, String insideContainerCode, Long userId, Long ouId) {
        ...
        // 0.判断是否已经缓存所有库存信息,获取库存统计信息及当前功能参数scan_pattern
   ...
   private String sysSuggestTipContainer(String containerCode, Long funcId, Integer putawayPatternDetailType, Long ouId, Long userId, String logId)  {
    ...
    public ScanResultCommand palletPutwayScanSku(WhSkuCommand skuCmd,String containerCode,Long ouId,Long userId,String locationCode,String insideContainerCode,Long functionId,Warehouse warehouse,Integer putawayPatternDetailType) {
            ...
            if (null == invList || 0 == invList.size()) {
                ...
                invList = pdaPutawayCacheManager.sysGuideContainerPutawayCacheInventoryAndStatistic(insideCommand, ouId, logId);
            }
            ...
            if (null == isCmd) {
                isCmd = pdaPutawayCacheManager.sysGuideContainerPutawayCacheInventoryStatistic(insideCommand, ouId, logId);
            }
            ContainerStatisticResultCommand csrCmd = new ContainerStatisticResultCommand();
            if (null != outCommand) {
            ...
            if(!skuIds.contains(skuId)) {
                log.error("sku is no exist, logId is:[{}]", logId);
                throw new BusinessException(ErrorCodes.SKU_IS_NOT_EXIST);
            }
            // 1.获取功能配置
            WhFunctionPutAway putawyaFunc = whFunctionPutAwayManager.findWhFunctionPutAwayByFunctionId(functionId, ouId, logId);
            if (null == putawyaFunc) {
    ...
    public ScanResultCommand containerPutwayScanSku(WhSkuCommand skuCmd,String containerCode,Long ouId,Long userId,String locationCode,String insideContainerCode,Long functionId,Warehouse warehouse,Integer putawayPatternDetailType){
        ...
        Long outContainerId = outCommand.getId();
        //整箱上架：判断内部容器是否可以上架
        Integer containerStatus = insideContainerCmd.getStatus();
        if (ContainerStatus.CONTAINER_STATUS_CAN_PUTAWAY != containerStatus && ContainerStatus.CONTAINER_STATUS_PUTAWAY != containerStatus) {
            log.error("container status is invalid, containerStatus is:[{}], logId is:[{}]", containerStatus, logId);
        ...
        containerList.add(insideContainerId);
        List<WhSkuInventoryCommand> skuList = whSkuInventoryDao.findWhSkuInventoryByInsideContainerId(ouId, containerList);
        if(null == skuList || skuList.size() == 0) {
            throw new BusinessException(ErrorCodes.SKU_IS_NOT_EXIST);    //查看扫描的商品在商品表中不存在
        }
        List<Long> skuListId = new ArrayList<Long>();
        for(WhSkuInventoryCommand wsCommand:skuList) {
           skuListId.add(wsCommand.getSkuId());
        }
        if(!skuListId.contains(whSkuCommand.getId())) {
            throw new BusinessException(ErrorCodes.SKU_IS_NOT_EXIST);    //查看扫描的商品在商品表中不存在
        }
        ScanResultCommand sCommand = new ScanResultCommand();
        InventoryStatisticResultCommand isrCmd = cacheManager.getMapObject(CacheConstants.CONTAINER_INVENTORY_STATISTIC, outContainerId.toString());
     ...
     * @param sCommand
     */
    private void updateContainerStatus(ContainerCommand outCmd,ContainerCommand insideCmd,Long ouId,Long userId,ScanResultCommand sCommand) {
        log.info("PdaSysSuggestPutwayManagerImpl updateContainerStatus is start"); 
        Long outId = null;
        if(null != outCmd) {
            outId = outCmd.getId();   //外部容器id
        }
        if(WhPutawayPatternDetailType.CONTAINER_PUTAWAY == sCommand.getPutawayPatternDetailType()) {  //整箱上架
        ...
        if(WhPutawayPatternDetailType.PALLET_PUTAWAY == sCommand.getPutawayPatternDetailType())  { //整托上架
            InventoryStatisticResultCommand isCmd = cacheManager.getMapObject(CacheConstants.CONTAINER_INVENTORY_STATISTIC,outId.toString());
            Set<Long> insideContainerIds = isCmd.getInsideContainerIds();   //整托上架内部容器id集合
            //修改外部容器状态
            Container container = new Container();
            BeanUtils.copyProperties(outCmd, container);
            container.setLifecycle(ContainerStatus.CONTAINER_LIFECYCLE_USABLE);
            ...
            //整托上架清除缓存
            this.sysSuggestPutwayRemoveAllCache(outCmd, logId);
        }
        if(WhPutawayPatternDetailType.SPLIT_CONTAINER_PUTAWAY == sCommand.getPutawayPatternDetailType()){
            Long insideId = insideCmd.getId();   //内部容器id
            if(null == outId && null != insideId) {  //拆箱上架，只有容器，没有托盘
                //修改内部容器状态
                Container container = new Container();
                BeanUtils.copyProperties(insideCmd, container);
                container.setLifecycle(ContainerStatus.CONTAINER_LIFECYCLE_USABLE);
                ...
                //整托上架清除缓存
                this.sysSuggestPutwayRemoveAllCache(outCmd, logId);
            }else{//拆箱上架内部容器状态修改的条件为所有库位循环扫描完毕且商品也扫描完毕，同样外部容器状态修改的条件是所有内容容器均已循环扫描完毕
                if(sCommand.isAfterPutawayTipContianer()) {   //托盘内还有内有上架的货箱,只更新货箱转态,不更新托盘状态
                    Container container = new Container();
                    BeanUtils.copyProperties(insideCmd, container);
                    container.setLifecycle(ContainerStatus.CONTAINER_LIFECYCLE_USABLE);
     ...
     * @param logId
     */
    private void sysSuggestPutwayRemoveAllCache(ContainerCommand containerCmd, String logId) {
        ...
        // 4.清楚所有提示库位缓存信息
        cacheManager.removeMapValue(CacheConstants.LOCATION_RECOMMEND, ocId.toString());
