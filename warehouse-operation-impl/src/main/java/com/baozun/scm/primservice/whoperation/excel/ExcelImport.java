...
public class ExcelImport extends AbstractExcelResolver {
    ...
    protected ExcelImportResult doReadExcel(ExcelDefinition excelDefinition, int titleIndex, InputStream excelStream, OutputStream outStream, Locale locale) throws Exception {
        Workbook workbook = WorkbookFactory.create(excelStream);
        ExcelImportResult result = new ExcelImportResult();
        result.setSheetName(excelDefinition.getSheetname());
        ...
        try {
            ...
            if (outStream != null) {
                ...
                // 封装excel读取错误信息至工作簿
                exportImportErroeMsg(workbook, e);
                result.setReadstatus(ExcelImportResult.READ_STATUS_FAILED);
                result.setRootExcelException(e);
                return result;
            }
        } finally {
            try {
    ...
    protected <T> List<T> readRows(ExcelDefinition excelDefinition, List<String> titles, Sheet sheet, int titleIndex, Locale locale) throws Exception {
     ...
     * @throws Exception
     */
    protected Object readRow(ExcelDefinition excelDefinition, Row row, List<String> titles, int rowNum, RootExcelException exception, Locale locale) throws Exception {
        // 创建注册时配置的bean类型
    ...
    private void validate(FieldValue fieldValue, Object value, int rowNum, RootExcelException rootExcelException, Locale locale) {
        if (value == null || StringUtils.isBlank(value.toString())) {
            ...
            if (!fieldValue.isNull()) {
                ...
                rootExcelException.getExcelExceptions().add(new ExcelException(errorMsg, fieldValue.getNullErrorCode(), rowNum, fieldValue.getTitle()));
            }
        } else {
            // 正则校验
            ...
            if (StringUtils.isNotBlank(regex)) {
                ...
                if (!val.matches(regex)) {
                    ...
                    rootExcelException.getExcelExceptions().add(new ExcelException(errorMsg, fieldValue.getRegexErrCode(), rowNum, fieldValue.getTitle()));
                }
            }
        }
    }
 ...
 throws EncryptedDocumentException, InvalidFormatException, IOException {
        ...
        for (ExcelException ee : rootExcelException.getExcelExceptions()) {
            ...
            if (ee.getTitleName() == null) {
                cell.setCellValue(cell.getStringCellValue() + ee.getMessage());
            } else {
