...
public class OutBoundBoxTypeManagerImpl implements OutBoundBoxTypeManager {
    ...
    public static final Logger log = LoggerFactory.getLogger(OutBoundBoxTypeManagerImpl.class);
    @Autowired
    ...
    @Override
    @MoreDB(DbDataSource.MOREDB_SHARDSOURCE)
    public OutBoundBoxType checkSkuBarcode(Long ouId, String consumableSkuBarcode) {
        List<OutBoundBoxType> outBoundBoxType = outBoundBoxTypeDao.findBySkuBarcode(ouId, consumableSkuBarcode);
        if (null == outBoundBoxType || outBoundBoxType.isEmpty()) {
            throw new BusinessException("没找到耗材");
        }
        if (outBoundBoxType.size() > 1) {
            throw new BusinessException("找到多个耗材");
        }
